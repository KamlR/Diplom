...
salaryController.get('/user-op-hash', AuthMiddleware.verifyToken, async (req: Request, res: Response) => {
...
salaryController.post('/sign-salary-userop', AuthMiddleware.verifyToken, async (req: Request, res: Response) => {
...
salaryController.put('/change-salary-date', AuthMiddleware.verifyToken, async (req: Request, res: Response) => {
  ...
  try {
    ...
    await cronJobsCollection.updateOne({ job: 'payroll' }, { $set: { schedule: cronSchedule } })
  } catch (error: any) {
    res.status(500).json({ error: error.message })
    return
  }
