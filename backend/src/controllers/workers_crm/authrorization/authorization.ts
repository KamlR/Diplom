...
authorizationController.post('/authorize', async (req: Request, res: Response) => {
  ...
  try {
    ...
    existWorker = await workersCrmCollection.findOne({ walletAddress: expectedWalletAddress })
    console.log(existWorker)
    if (existWorker == null) {
      await workersCrmCollection.insertOne({ walletAddress: expectedWalletAddress, role: role })
    }
  } catch (error) {
  ...
  const accessToken = jwt.sign({ expectedWalletAddress }, process.env.ACCESS_TOKEN_KEY, { expiresIn: '10h' })
  const refreshToken = jwt.sign({ expectedWalletAddress }, process.env.REFRESH_TOKEN_KEY, { expiresIn: '7d' })
  ...
  res.status(200).json({ accessToken: accessToken, existWorker: existWorker == null ? false : true })
})

authorizationController.post('/add_info', AuthMiddleware.verifyToken, async (req: Request, res: Response) => {
  ...
  try {
    const db = await connectToDatabase()
    const workersCrmCollection = db.collection('workers_crm')
    await workersCrmCollection.updateOne({ walletAddress }, { $set: { lastName, firstName } })
    res.status(200).send()
  } catch (error) {
...
authorizationController.get('/role', AuthMiddleware.verifyToken, async (req: Request, res: Response) => {
  const walletAddress = req.params.walletAddress
  const db = await connectToDatabase()
  const workersCrmCollection = db.collection('workers_crm')
...
function checkWalletAddress(expectedWalletAddress: string, message: string, signature: string): boolean {
  try {
